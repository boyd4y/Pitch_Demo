...
void listeningCallback(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer, const AudioTimeStamp *inStartTime, UInt32 inNumberPacketsDescriptions, const AudioStreamPacketDescription *inPacketDescs) 
{
	...
	if ([listener isListening])
    {
        ...
        {
            [listener.delegate BufferFullCallback:inBuffer withPacketDesc:inPacketDescs];
        }
    }
}
...
- (void)setupQueue {
	...
	[self setupFormat];
    AudioQueueNewInput(&format, listeningCallback, self, CFRunLoopGetCurrent(), kCFRunLoopCommonModes, 0, &queue);
	[self setupBuffers];
	[self setupMetering];	
}
...
- (void)setupBuffers {
	...
	for (NSInteger i = 0; i < 3; ++i) { 
		AudioQueueAllocateBuffer(queue, 8192, &buffers[i]); 
